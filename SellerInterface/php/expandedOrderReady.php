...
//query orders, order_items, item tables using oid and iid
$select = "SELECT orders.*, location.*, order_items.qty, item.description
           FROM orders
           JOIN location ON orders.lid = location.lid
           JOIN order_items ON orders.oid = order_items.oid
           JOIN item ON order_items.iid = item.iid
...
if ($result->num_rows > 0) {
    ...
    //form for X button
    echo "<form method='post' action='orderCompleted.php' style='display: inline-block;'>";
